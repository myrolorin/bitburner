...
export class Server {
  ...
  maxHackThreads() {
    return Math.floor(this.freeRam() / getScriptRam(this.hackScript, this.hostname))
  }

  maxGrowThreads() {
    return Math.floor(this.freeRam() / getScriptRam(this.growScript, this.hostname))
  }

  maxWeakenThreads() {
    return Math.floor(this.freeRam() / getScriptRam(this.weakenScript, this.hostname))
  }
  ...
  hasScripts() {
    return this.fileExists(hackScript)
        && this.fileExists(growScript)
        && this.fileExists(weakenScript)
  }
  ...
  async hack(target, threads, delay) {
    await this.ns.exec(this.hackScript, this.hostname, threads, target, delay)
  }

  async grow(target, threads, delay) {
    await this.ns.exec(this.growScript, this.hostname, threads, target, delay)
  }

  async weaken(target, threads, delay) {
    await this.ns.exec(this.weakenScript, this.hostname, threads, target, delay)
  }
  ...
  minSecurity() {
    return this.ns.getServerMinSecurity(this.hostname)
  }
