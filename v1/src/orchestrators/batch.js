...
export class BatchOrchestrator {
  ...
  async deploy() {
    try {
      ...
      this.logger.debug(`Found #${rootedServers.length} rooted servers.`);
      ...
      this.logger.debug(`Filtered targets: ${targets.length} (${targets.join(", ")})`);
      ...
      for (const target of targets) {
        ...
        // Check if prepped
        const prepped = info.securityLevel <= info.minSecurity + 0.5 &&
                        info.moneyAvailable >= info.maxMoney * 0.99;
      ...
      this.logger.debug(`Built ${plans.length} attack plans.`);
  ...
  async deployPlanToServer(plan, target, server) {
    ...
    if (plan.type === "batch") {
      ...
      if (plan.hack)    ns.exec(this.hackScript, server, plan.hack.threads, target, plan.hack.delay);
      if (plan.weaken1) ns.exec(this.weakenScript, server, plan.weaken1.threads, target, plan.weaken1.delay);
      if (plan.grow)    ns.exec(this.growScript, server, plan.grow.threads, target, plan.grow.delay);
      if (plan.weaken2) ns.exec(this.weakenScript, server, plan.weaken2.threads, target, plan.weaken2.delay);
