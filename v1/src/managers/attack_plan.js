...
export class AttackPlan {
  ...
  constructor(ns, target, info, config = {}) {
    ...
    this.logger = new Logger(ns, `[AttackPlan:${target}] `);
  }
  ...
  build() {
    ...
    this.logger.debug(`Building plan for ${this.target}`);
    ...
    if (!prepped) {
      if (info.securityLevel > info.minSecurity + 0.5) {
        this.logger.debug(`[AttackPlan] ${this.target}: needs weaken`);
        ...
        this.logger.debug(`[AttackPlan] ${this.target}: needs grow`);
        ...
        this.logger.debug(`[AttackPlan] ${this.target}: not prepped, but not eligible for grow/weaken`);
      }
      this.expectedProfit = 0;
      return this;
    }
