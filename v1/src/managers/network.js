...
export class NetworkMap {
  ...
  async mapAndRoot() {
    ...
    try {
      ...
      // Refresh live server info for all mapped servers
      for (const [server, info] of this.serverMap) {
        info.securityLevel = this.ns.getServerSecurityLevel(server);
        info.moneyAvailable = this.ns.getServerMoneyAvailable(server);
        info.rooted = this.ns.hasRootAccess(server);
        info.maxMoney = this.ns.getServerMaxMoney(server);
        info.minSecurity = this.ns.getServerMinSecurityLevel(server);
        info.maxRam = this.ns.getServerMaxRam(server);
        info.usedRam = this.ns.getServerUsedRam(server);
        info.hackTime = this.ns.getHackTime(server);
        info.growTime = this.ns.getGrowTime(server);
        info.weakenTime = this.ns.getWeakenTime(server);
      }

      this.configManager.config["server-map"] = Object.fromEntries(this.serverMap);
